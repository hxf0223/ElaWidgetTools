...
ElaXIO_Interface::~ElaXIO_Interface() {
  if (_isInit) {
    ...
    std::cout << "xio_interface: Terminated."
              ...
              << " Received " << recvBytes << " bytes at " << recvRate << " bytes/second" << std::endl;
    _threadedIO.Stop();
    _threadedIO.Join();
...
bool ElaXIO_Interface::_connectToTarget(UDP_Target& aTarget) {
  ...
  if (aTarget._type == Broadcast || aTarget._type == Unicast) {
    ...
    if (!connected) {
      ...
      ElaXIO_Connection* connectionPtr =
          new ElaXIO_Connection(this, new PakUDP_IO(udpIO.release(), this, mUDP_HeaderPtr->Clone()));
...
std::unique_ptr<ElaXIO_Interface::ConnectionCallback> ElaXIO_Interface::disconnectConnect(
    ElaXIO_Connection* aConnectionPtr, const ConnectionCallback::FunctionType& aFunction) {
    ...
    if (mShowTransferRate) {
      ...
      if (mShowTransferRate) {
        std::cout << "xio_interface: Advanced time."
                  ...
                  << " Received " << recvBytes << " bytes at " << newRecvBytes / mHeartbeatInterval << " bytes/second"
                  << std::endl;
      }
    }
  }
  ...
  if (pkt._tcpPort != 0) {
    if (mProcessedHeartbeats.find(pkt._applicationId) == mProcessedHeartbeats.end()) {
      if (FindConnection(pkt._applicationId) == nullptr) {
        ...
        std::cout << "xio_interface: Received heartbeat. " << "Attempting to connect to " << static_cast<std::string>(address)
                  << std::endl;
...
void ElaXIO_Interface::_handleInit(ElaXIO_InitializePkt& pkt) {
  ...
  if (ok) {
    ...
    if (pkt._stage >= cCONNECT_STAGE) {
      ...
      std::cout << "xio_interface: Connected to application. " << "Application: " << connectionPtr->getApplicationName()
                << std::endl;
...
void ElaXIO_Interface::_handleDisconnect(PakSocketIO* socketIO, PakConnection* connection) {
  ...
  if (connectionPtr->isInitialized() && connectionPtr->isReliable()) {
    std::cout << "xio_interface: Disconnected from application. " << "Application: " << connectionPtr->getApplicationName()
              << std::endl;
  }
