  ...
  if (mErrorStatus == GenBuf::NoError) {
    ...
    if (aStatus > GenBuf::FixFailed) {
      if (GenError::ShowError(GenError::cIO_ERRORS)) {
        const char* msg[] = {"Experienced an internal GenI::HandleGetError", "Experienced an undefined error",
                             "Could find no more data to read", "Has no more room to put", "Had an allocation error"};

        std::cout << "GenI called HandleGetError. "
                  << "Wall Clock: " << UtWallClock::GetWallClockTime() << " GenI: " << mName << " Error: " << msg[-aStatus]
                  << " when trying to read " << aVariableSize << " bytes." << std::endl;
      }
    }
  }
