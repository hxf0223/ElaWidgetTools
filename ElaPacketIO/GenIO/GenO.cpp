  ...
  if (mErrorStatus == GenBuf::NoError) {
    ...
    if (aStatus > GenBuf::FixFailed) {
      if (GenError::ShowError(GenError::cIO_ERRORS)) {
        const char* msg[] = {"Experienced an internal GenO::HandlePutError", "Experienced an undefined error",
                             "Could find no more data to read", "Has no more room to put", "Had an allocation error"};

        std::cout << "GenO called HandlePutError."
                  << " Wall Clock: " << UtWallClock::GetWallClockTime() << " GenO: " << mName << " Error: " << msg[-aStatus]
