...
PakProcessor::PacketInfo* PakProcessor::RegisterPacketP(int aPacketId, const std::string aPacketName,
                                                        PacketCallbackList* aCallbackListPtr, bool aIsUndefined,
                                                        int aPacketBaseId) {
  ...
  if (pInfo != nullptr) {
    {  // RAII block
      std::cout << "Packet already initialized, cannot RegisterPacket again."
                << " Name: " << aPacketName << " ID: " << aPacketId << std::endl;
    }
    exit(0);
  }
...
PakProcessor::PacketInfo::PacketInfo(int aPacketId, std::string aPacketName, PacketCallbackList* aCallbackListPtr,
                                     bool aIsUndefined)
    : mPacketID(aPacketId),
