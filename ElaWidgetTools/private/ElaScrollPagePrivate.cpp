...
void ElaScrollPagePrivate::onNavigationRoute(QVariantMap routeData) {
  ...
  if (pageCheckSumKey == "Navigation") {
    QString pageTitle =
        isRouteBack ? routeData.value("ElaBackPageTitle").toString() : routeData.value("ElaForwardPageTitle").toString();
    q->navigation(_centralWidgetMap.value(pageTitle), false);
  } else if (pageCheckSumKey == "BreadcrumbClicked") {
    QStringList breadcrumbList = isRouteBack ? routeData.value("ElaBackBreadcrumbList").toStringList()
                                             : routeData.value("ElaForwardBreadcrumbList").toStringList();
