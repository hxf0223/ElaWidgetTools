...
void ElaRollerPickerPrivate::onRollerPickerClicked() {
  ...
  QPoint targetPos(q->mapToGlobal(
      QPoint(-6, (q->height() - _rollerPickerContainer->height() + _rollerPickerContainer->getButtonAreaHeight()) / 2)));
  _rollerPickerContainer->show();
  _rollerPickerContainer->setGeometry(
      QRect(targetPos, QSize(_getRollerTotalWidth() + 16, _pickerRollerHeight + _rollerPickerContainer->getButtonAreaHeight())));
