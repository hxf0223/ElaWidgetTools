...
void ElaReminderCard::paintEvent(QPaintEvent* event) {
  ...
  if (!d->_pCardPixmap.isNull()) {
    ...
    if (d->_pCardPixMode == ElaCardPixType::PixMode::Ellipse) {
      path.addEllipse(QPointF(d->_pCardPixmapSize.width() / 2 + pixLeftMargin, height() / 2), d->_pCardPixmapSize.width() / 2,
      ...
      path.addRoundedRect(QRectF(pixLeftMargin, (height() - d->_pCardPixmapSize.height()) / 2, d->_pCardPixmapSize.width(),
