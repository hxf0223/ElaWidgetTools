...
ElaPopularCard::ElaPopularCard(QWidget* parent)
    : QWidget{parent},
      d_ptr(new ElaPopularCardPrivate()) {
  Q_D(ElaPopularCard);
...
void ElaPopularCard::paintEvent(QPaintEvent* event) {
  ...
  // 图片绘制
  painter.save();
  QRectF pixRect(foregroundRect.x() + foregroundRect.height() * 0.15, foregroundRect.y() + foregroundRect.height() * 0.15,
  ...
  QRectF titleRect(pixRect.right() + d->_textHSpacing, pixRect.y(),
                   d->_floater->_floatGeometryOffset * 2 + foregroundRect.width() - pixRect.width() - d->_textHSpacing * 2 -
                       foregroundRect.height() * 0.15 - buttonTargetWidth,
                   titleHeight);
  ...
  if (!d->_pInteractiveTips.isEmpty()) {
    ...
    painter.setFont(font);
    // 覆盖背景绘制
    QRectF tipRect(foregroundRect.right() - d->_textHSpacing - tipWidth, foregroundRect.bottom() - d->_textHSpacing - tipHeight,
    ...
    QRectF tipRect(foregroundRect.right() - d->_textHSpacing - 50, foregroundRect.bottom() - d->_textHSpacing - tipHeight,
                   foregroundRect.width() / 2 - d->_textHSpacing, tipHeight);
