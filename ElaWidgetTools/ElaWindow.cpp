...
ElaWindow::ElaWindow(QWidget* parent)
  ...
  // 返回按钮状态变更
  connect(ElaNavigationRouter::getInstance(), &ElaNavigationRouter::navigationRouterStateChanged, d,
          &ElaWindowPrivate::onNavigationRouterStateChanged);
...
void ElaWindow::setCustomWidget(ElaAppBarType::CustomArea customArea, QWidget* widget, QObject* hitTestObject,
...
#endif
  setGeometry((geometry.left() + geometry.right() - width()) / 2, (geometry.top() + geometry.bottom() - height()) / 2, width(),
              height());
}
...
ElaNavigationType::NodeOperateReturnType ElaWindow::addFooterNode(QString footerTitle, QWidget* page, QString& footerKey,
                                                                  int keyPoints, ElaIconType::IconName awesome) {
  Q_D(ElaWindow);
    ...
    case ElaApplicationType::Normal: {
      switch (d->_pWindowPaintMode) {
        ...
        case ElaWindowType::Pixmap: {
          ...
          painter.drawPixmap(
              rect(), *pix,
              QRect((pix->width() - targetPixWidth) / 2, (pix->height() - targetPixHeight) / 2, targetPixWidth, targetPixHeight));
          break;
        }
