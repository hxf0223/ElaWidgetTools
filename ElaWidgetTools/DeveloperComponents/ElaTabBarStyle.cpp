...
void ElaTabBarStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter,
  ...
  switch (element) {
    ...
    case QStyle::CE_TabBarTabShape: {
      ...
      if (const QStyleOptionTab* topt = qstyleoption_cast<const QStyleOptionTab*>(option)) {
        ...
        if (topt->state.testFlag(QStyle::State_Selected)) {
          ...
          painter->setBrush(topt->state.testFlag(QStyle::State_Sunken) ? ElaThemeColor(_themeMode, BasicSelectedHover)
                                                                       : ElaThemeColor(_themeMode, BasicSelectedAlpha));
          QPainterPath path;
    ...
    case QStyle::CE_TabBarTabLabel: {
      ...
      if (const QStyleOptionTab* topt = qstyleoption_cast<const QStyleOptionTab*>(option)) {
        ...
        if (!icon.isNull()) {
          QRectF iconRect(topt->rect.x() + 15, textRect.center().y() - (qreal)topt->iconSize.height() / 2 + 1,
...
QSize ElaTabBarStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size,
                                       const QWidget* widget) const {
  switch (type) {
    case CT_TabBarTab: {
      return _pTabSize;
