...
void ElaRollerPickerContainer::paintEvent(QPaintEvent* event) {
  ...
  if (!_animationPix.isNull()) {
    ...
    // 按钮区域绘制
    _overButtonRect =
        QRect(foregroundRect.x() + _buttonMargin, foregroundRect.bottom() - _pButtonAreaHeight + _buttonMargin,
              (foregroundRect.width() - 2 * _buttonMargin - _buttonSpacing) / 2, _pButtonAreaHeight - 2 * _buttonMargin + 1);
    _cancelButtonRect =
        QRect(_overButtonRect.right() + _buttonSpacing, _overButtonRect.y(), _overButtonRect.width(), _overButtonRect.height());
    // 按钮覆盖绘制
