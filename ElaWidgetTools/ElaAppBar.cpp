...
ElaAppBar::ElaAppBar(QWidget* parent)
  ...
  Q_D(ElaAppBar);
  d->_buttonFlags = ElaAppBarType::RouteBackButtonHint | ElaAppBarType::RouteForwardButtonHint |
                    ElaAppBarType::StayTopButtonHint | ElaAppBarType::ThemeChangeButtonHint | ElaAppBarType::MinimizeButtonHint |
...
void ElaAppBar::setCustomWidget(ElaAppBarType::CustomArea customArea, QWidget* widget, QObject* hitTestObject,
...
void ElaAppBar::closeWindow() {
  ...
  geometryAnimation->setEndValue(
      QRectF(geometry.center().x() - targetWidth / 2, geometry.center().y() - targetHeight / 2, targetWidth, targetHeight));
...
void ElaAppBar::paintEvent(QPaintEvent* event) {
  if (eWinHelper->getIsWinVersionGreater10() && !eWinHelper->getIsWinVersionGreater11()) {
    ...
    painter.setPen(QPen(window()->isActiveWindow() ? ElaThemeColor(d->_themeMode, Win10BorderActive)
                                                   : ElaThemeColor(d->_themeMode, Win10BorderInactive),
                        borderWidth));
